<template>
	...
	>
		...
		<div
			id="kinect-cursor"
			:style="cursorStyle"
			class="fixed top-0 left-[700px] z-50 w-8 h-8 rounded-full pointer-events-none transition-all ease-linear shadow-lg bg-red-600"
		/>
				...
				>
					...
					<div class="flex flex-col items-center justify-center w-80 gap-4">
						<div v-for="option in questionOptions.Q2" :key="option" class="w-full">
							<button
								class="kinect-interactive bg-[#E6FFE9] border-4 border-[#29983C] rounded-full w-full h-20 hover:scale-110 transition-transform duration-300 p-4"
				...
				>
					...
					<div class="flex flex-col items-center justify-center w-80 gap-4">
						<div v-for="option in questionOptions.Q3" :key="option" class="w-full">
							<button
								class="kinect-interactive bg-[#FFFBF4] border-4 border-[#FFAD08] rounded-full w-full h-20 hover:scale-110 transition-transform duration-300 p-4"
								@click="handleOptionSelect(option)"
							>
								{{ option }}
							</button>
				...
				>
					...
					<div class="flex flex-col items-center justify-center w-80 gap-4">
						<div v-for="option in questionOptions.Q4" :key="option" class="w-full">
							<button
								class="kinect-interactive bg-[#E7F3FF] border-4 border-[#167EE7] rounded-full w-full h-20 hover:scale-110 transition-transform duration-300 p-4"
								@click="handleOptionSelect(option)"
							>
								{{ option }}
							</button>
		...
		<div v-if="isVideo">
			...
			>
				...
				<button
					class="kinect-interactive bg-black text-white z-50 p-2 absolute top-10"
					@click="print"
				>
					END
				</button>
...
const handleClick = () => {
	...
	const isClickable =
		element && element.classList.contains("kinect-interactive");

	if (isClickable && element === latestHoverElement) {
		count++;

		console.log("Count:", count);

		if (count >= 5) {
			console.log("Kinect Clicked:", element);
			element.click();
			count = 0;
			latestHoverElement = null;
		}
	} else {
		latestHoverElement = element;
		count = 0;
	}
};

watch([cursorX, cursorY], handleClick);
...
const handleStateChange = () => {
	state.value++;
    state.value = state.value % 3; // 確保狀態在 0-3 之間循環
...
const URL = "http://192.168.0.110:4000/api/print";
...
const print = async () => {
	...
	console.log("Print request sent, response:", response);
};
